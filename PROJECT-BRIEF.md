...
- **Preserve the legacy `throughthelenzphoto.com` domain** (and its `www` version) as a redirect-only domain once DNS is switched — full procedure and ready-to-deploy redirect rules documented in `FULL-SITE-AUDIT.md` section 4.1. Not implemented yet; no DNS changes made.
...
- **Google Search Console** — register the production domain (and submit `sitemap.xml`) so Google indexes the new site correctly and so Sarah can monitor indexing status, organic-search impressions, clicks, search queries, and per-page search performance after launch. Also covers crawl-error monitoring.
- **Sitemap submission** — submit the auto-generated sitemap.xml to Google (and Bing) once the domain is live.
- **Analytics — decided: Cloudflare Web Analytics.** Cookieless/no consent banner, gives exactly what Sarah wants (total visitors, page views, most-viewed pages, referral sources, basic geography) without the overhead of GA4. The code side is already wired: `siteConfig.analytics.cloudflareBeaconToken` in `src/lib/site-config.ts` is `null`, and `BaseLayout.astro` only renders the Cloudflare beacon script when that's filled in. Once the production domain is live: add it under Cloudflare's dashboard → Web Analytics (works even without proxying DNS through Cloudflare — it just issues a per-hostname beacon token), paste that token into `cloudflareBeaconToken`, redeploy, and confirm real pageviews are landing in the Cloudflare dashboard before considering this done. Do not add Google Analytics/GA4 unless Sarah later asks for deeper conversion/event-level analysis.
